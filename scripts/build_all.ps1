...
.DESCRIPTION
    ...
    compiles Go code into WebAssembly (GOOS=wasip1 GOARCH=wasm), and packages them
    into production-ready .actonpkg archives in ./dist.
...
foreach ($manifestItem in $PluginManifests) {
    ...
    $pkgOutput  = Join-Path $DistDir "$pluginId.actonpkg"
    ...
    $sw.Stop()
    $pkgSizeKB = [math]::Round(((Get-Item $pkgOutput).Length / 1024), 1)

    Write-Host "   ✅ Compiled & Packaged -> dist/$pluginId.actonpkg ($pkgSizeKB KB) in $($sw.ElapsedMilliseconds)ms" -ForegroundColor Green
...
Write-Host "All distributable packages are located in: $DistDir" -ForegroundColor Cyan
Write-Host "=================================================================" -ForegroundColor Cyan
